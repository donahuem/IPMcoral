...
##Plot Kernel by site
#find minimums and maximums by site for transition probability legend
mins<-rep(0,10)
maxs<-rep(0,10)
for (site in 1:nx){
  K<-Kernel(y,n,params)
...
#find highest maximum and lowest minimum for transition probability legend
ma<-max(maxs)
mi<-min(mins)
...
#Plot single Kernel
par(oma=c(6,6,1,10))
site=6 #can change site for different site's plot
...
#Plots for All sites parameter elasticities by parameter
par(mfrow = c(2, 2))
par(mar = c(7, 2, 2, 2) + 0.1)
for (i in c(1:ncoef)){
...
for (i in 1:32){
  ...
  plot(eig,elasP[,i],col=cl,main=rownames(corelas)[i],pch=18)
}
...
###############################################################################################
####look for correlation between parameter elasticities and cover
#Get CRAMP transect cover data
...
#Plot trajectories in M. capitata cover from 1999-2012 (CRAMP transects)
par(mfrow=c(3,4))
for(i in 1:10){
...
#Determine whether slope of coral cover over time is significant at alpha=0.05
...
#Make matrix to store r and p-value from correlation test
...
for (i in 1:32){
  ...
  plot(slopes$slopes,elasP[,i],main=rownames(corelas1)[i],pch=18, col=cl)
}

#no significant relationships between slope of cover with time and elasticitiy parameters
sigonly.slope.elas<-subset(corelas1,corelas1[,2]<0.05)

#Try again using only areas with significant changes in cover
slopes1<-slopes[c(2,4,6,7,9),]
corelas1<-matrix(0,32,2)
rownames(corelas1)<-rownames(params)
colnames(corelas1)<-c("r","pvalue")
par(mfrow=c(3,4))
...
sigonly.slope.elas.sig<-subset(corelas1,corelas1[,2]<0.05)

#significant relationship with recruit.size.sd and r
#####################################################################
#Try with truncated CRAMP transect data
#Plot cover vs time for M. capitata cover from 1999-2005 (this is the itme period that we have data for)
...
for(i in 1:10){
  ...
  plot(x$year,x$cover,pch=18,main=levels(MCCtrunc$utrans)[i],xlab="year",ylab="M. capitata cover",col=cl)
  bleh<-lm(cover~year,data=x)
  dyear<-data.frame(year=x$year)
  lines(x$year,predict(bleh,newdata=dyear))
}
...
sigonly.Truncslope.lambda<-subset(corelas1,corelas1[,2]<0.05)
