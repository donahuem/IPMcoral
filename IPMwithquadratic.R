...
# growth regression
growth.reg<-gls(sizeNext~size*utrans+size*fate+size2,weights=varExp(form=~size),data=MCg)
summary(growth.reg)

growth.int<-c(coefficients(growth.reg)[1],coefficients(growth.reg)[1]+coefficients(growth.reg)[3:(nx+1)])
growth.slope1<-c(coefficients(growth.reg)[2],coefficients(growth.reg)[2]+coefficients(growth.reg)[15:23])
growth.slope2<-coefficients(growth.reg)[14]
growth.sd2=((summary(growth.reg)$sigma))^2
growth.varFunc=coef(growth.reg$modelStruct$varStruct,"exp")

fiss.int<-c(growth.int+coefficients(growth.reg)[12])
fiss.slope1<-c(growth.slope1+coefficients(growth.reg)[24])
fuse.int<-c(growth.int+coefficients(growth.reg)[13])
fuse.slope1<-c(growth.slope1+coefficients(growth.reg)[25])
...
params[4,]<-c(growth.slope1)            #growth slope by site
params[5,]<-rep(growth.slope2)
params[6,]<-rep(growth.varFunc)
params[7,]<-rep(growth.sd2)            #variance in growth, same across sites
params[8,]<-c(fiss.int)                #fiss growth intercept by site
params[9,]<-c(fiss.slope1)              #fiss growth slope by site
params[10,]<-c(fuse.int)                #fuse growth intercept by site
params[11,]<-c(fuse.slope1)             #fuse growth slope by site
params[12,]<-c(Pfiss.int)              #Probability of fission intercept by site
params[13,]<-c(Pfiss.slope)          #Probability of fission slope, same across sites
params[14,]<-c(Pfuse.int)              #Probability of fusion intercept by site
params[15,]<-rep(Pfuse.slope)            #Probability of fusion slope by site
params[16,]<-c(PfuseLG.int)            #Probability of being large fusion intercept given that you survived and fused by site
params[17,]<-rep(PfuseLG.slope)        #probability of being a large fusion slope given that you survived and fused same across sites
params[18,]<-rep(clonesize.int)        #clone size intercept, same across sites
params[19,]<-rep(clonesize.slope)      #clone size slope, same across sites
params[20,]<-rep(clonesize.sd2)        #clone size sd, same across sites
params[21,]<-rep(clonesize.varFunc)    #clone size variance increases with size by site
params[22,]<-rep(clonenumlo.int)     #clonenum intercept lo number of fission events 
params[23,]<-rep(clonenumlo.slope)   #clonenum slope lo number of fission events
params[24,]<-rep(clonenumhi.int)     #clonenum intercept hi number of fission events
params[25,]<-rep(clonenumhi.slope)   #clonenum slope hi number of fission events
params[26,]<-rep(Philo.int)
params[27,]<-rep(Philo.slope)
...
params[30,]<-c(r)
rownames(params)<-c("surv.int","surv.slope","growth.int","growth.slope1","growth.slope2","growth.varFunc","growth.sd2","fiss.int","fiss.slope1","fuse.int","fuse.slope1","Pfiss.int","Pfiss.slope","Pfuse.int","Pfuse.slope","PfuseLG.int","PfuseLG.slope","clonesize.int","clonesize.slope","clonesize.sd2","clonesize.varFunc","clonenumlo.int","clonenumlo.slope","clonenumhi.int","clonenumhi.slope","Philo.int","Philo.slope","recruit.size.mean","recruit.size.sd","r")
...
gg.yx=function(xp,x,params) {
  sigmaxp2<-params[7,site]*exp(2*params[6,site]*x)
  sigmaxp<-sqrt(sigmaxp2)
  g<-dnorm(xp,params[3,site]+params[4,site]*x+params[5,site]*x^2,sigmaxp)
}
ig.yx=function(xp,x,params) {
  sigmaxp2<-params[7,site]*exp(2*params[6,site]*x)
  sigmaxp<-sqrt(sigmaxp2)
  g<-dnorm(xp,params[8,site]+params[9,site]*x+params[5,site]*x^2,sigmaxp)
}
ug.yx=function(xp,x,params) {
  sigmaxp2<-params[7,site]*exp(2*params[6,site]*x)
  sigmaxp<-sqrt(sigmaxp2) 
  g<-dnorm(xp,params[10,site]+params[11,site]*x+params[5,site]*x^2,sigmaxp)
}


# probability of fissing     
p.fiss.x=function(x,params) {
  u=exp(params[12,site]+params[13,site]*x)
  return(u/(1+u))
}
# probability of fusion
p.fuse.x=function(x,params) {
  u=exp(params[14,site]+params[15,site]*x)
  return(u/(1+u))
}
...
clo.yx=function(xp,x,params) {
  sigmaxp2<-params[20,site]*exp(2*params[21,site]*x)
  sigmaxp<-sqrt(sigmaxp2)
  nfiss<-exp(params[22,site]+params[23,site]*x)
  ...
  p.fiss.x(x,params)*p.hilo.x(x,params)*nfiss*dnorm(xp,params[18,site]+params[19,site]*x,sigmaxp)
}
chi.yx=function(xp,x,params) {
  sigmaxp2<-params[20,site]*exp(2*params[21,site]*x)
  sigmaxp<-sqrt(sigmaxp2)
  nfiss<-exp(params[24,site]+params[25,site]*x)
  ...
  p.fiss.x(x,params)*(1-p.hilo.x(x,params))*nfiss*dnorm(xp,params[18,site]+params[19,site]*x,sigmaxp)
}
...
f.yx=function(xp,x,params){
  params[30,site]*x*dnorm(xp,params[28,site],params[29,site])
}
