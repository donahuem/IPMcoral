...
attach(MCr)
hist(recruits)
histogram(~recruits|utrans,type="count")
...
rcrt.glmU_D <- glm(recruits~utrans + lnspcover,family=poisson)
#AIC=616; this is a bit better (del AIC is 7); overdispersion: 376/210 = 1.79
rcrt.glmUD <- glm(recruits~utrans*lnspcover,family=poisson)
#AIC=618; overdispersion:  363/202=1.99
rcrt.glmU_D2 <- glm(recruits~utrans + lnspcover + lnspcover2,family=poisson)
...
rcrt.glmUP <- glm(recruits~utrans*pspcover,family=poisson)
#AIC=655, phi=401/202=1.99
rcrt.glmU_P <- glm(recruits~utrans+pspcover,family=poisson)
#AIC=670
rcrt.glmU_P2 <- glm(recruits~utrans+pspcover+pspcover2,family=poisson)
...
hist(MCr$sizeNext,ylim=c(0,1),freq=FALSE)
rec.size<-lm(sizeNext~1,MCr)
rec.size.U<-lm(sizeNext~utrans,MCr)
AIC(rec.size,rec.size.U)
for (i in 2:10){
lines(x,dnorm(x,coefficients(rec.size.U)[1]+coefficients(rec.size.U)[i],summary(rec.size.U)$sigma))}
histogram(~MCr$sizeNext|MCr$utrans)
