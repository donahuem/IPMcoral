...
par(mfrow=c(3,1),mar=c(5,2,2,10)+0.1,xpd=T)
plot(growdat$size,growdat$sizeNext,col=cl[growdat$utrans],type="p",pch=18,cex=0.65,main="Stasis",xlab="Size (t)",ylab="Size (t+1)",ylim=c(-2,10),xlim=c(-2,10))
for (site in 1:(nx)) {
  lines(x,params[3,site]+params[4,site]*x,col=cl[site],lwd=1)
}

plot(growdat$size,growdat$sizeNext,col=cl[growdat$utrans],type="p",pch=18,cex=0.65,main="Fission",xlab="Size (t)",ylab="Size (t+1)",ylim=c(-2,10),xlim=c(-2,10))
for (site in 1:(nx)) {
  lines(x,params[7,site]+params[8,site]*x,col=cl[site],lwd=1)
}

plot(growdat$size,growdat$sizeNext,col=cl[growdat$utrans],type="p",pch=18,cex=0.65,main="Fusion",xlab="Size (t)",ylab="Size (t+1)",ylim=c(-2,10),xlim=c(-2,10))
for (site in 1:(nx)) {
  lines(x,params[9,site]+params[10,site]*x,col=cl[site],lwd=1)
}

legend(13.2,10, legend = sprintf(levels(growdat$utrans)), col = cl, lwd=2, xjust = 1, bg = "white")
...
for (site in 1:10){
  ...
  K<-Kernel(y,n,params)
  image(y,y,t(K), col=topo.colors(300),zlim=c(mi,0.07),main=sn[site],xlab="",ylab="",axes=FALSE)
...
vertical.image.legend(col=topo.colors(300),zlim=c(mi,0.07))
