...
ncoef<-42 # number of coefficients
...
# growth regression
growthngsvar<-gls(sizeNext~size*utrans,GONGS,weights=varExp(form=~size))
summary(growthngsvar)
growthngs.int<-c(coefficients(growthngsvar)[1],coefficients(growthngsvar)[1]+coefficients(growthngsvar)[3:11])
growthngs.slope<-c(coefficients(growthngsvar)[2],coefficients(growthngsvar)[2]+coefficients(growthngsvar)[12:20])
growthngs.sd2<-((summary(growthngsvar)$sigma))^2
growthngs.varFunc<-coef(growthngsvar$modelStruct$varStruct,"exp")

growthes<-lm(sizeNext~size,GOES)
summary(growthes)
growthes.int<-coefficients(growthes)[1]
growthes.slope<-coefficients(growthes)[2]
growthes.sd<-summary(growthes)$sigma

growtheg<-lm(sizeNext~size,GOEG)
summary(growtheg)
growtheg.int<-coefficients(growtheg)[1]
growtheg.slope<-coefficients(growtheg)[2]
growtheg.sd<-summary(growtheg)$sigma
...
Es.full<-glm(PES~size,family=binomial,data=growthonly)
Eg.full<-glm(PEG~size,data=growthonly,family=binomial)

Pes.int<-coefficients(Es.full)[1]
Pes.slope<-coefficients(Es.full)[2]

Peg.int<-coefficients(Eg.full)[1]
Peg.slope<-coefficients(Eg.full)[2]
...
params[3,]<-c(growthngs.int)              #growth intercept by site
params[4,]<-c(growthngs.slope)            #growth slope by site
params[5,]<-rep(growthngs.sd2)
params[6,]<-rep(growthngs.varFunc)
params[7,]<-rep(growthes.int)              #growth intercept by site
params[8,]<-rep(growthes.slope)            #growth slope by site
params[9,]<-rep(growthes.sd)
params[10,]<-rep(growtheg.int)              #growth intercept by site
params[11,]<-rep(growtheg.slope)            #growth slope by site
params[12,]<-rep(growtheg.sd)            #variance in growth, same across sites
params[13,]<-rep(fiss.int)                #fiss growth intercept by site
params[14,]<-rep(fiss.slope)              #fiss growth slope by site
params[15,]<-rep(fiss.sd)
params[16,]<-rep(fuse.int)                #fuse growth intercept by site
params[17,]<-rep(fuse.slope)             #fuse growth slope by site
params[18,]<-rep(fuse.sd2)
params[19,]<-rep(fuse.varFunc)
params[20,]<-rep(Pes.int)
params[21,]<-rep(Pes.slope)
params[22,]<-rep(Peg.int)
params[23,]<-rep(Peg.slope)
params[24,]<-c(Pfiss.int)              #Probability of fission intercept by site
params[25,]<-c(Pfiss.slope)          #Probability of fission slope, same across sites
params[26,]<-c(Pfuse.int)              #Probability of fusion intercept by site
params[27,]<-rep(Pfuse.slope)            #Probability of fusion slope by site
params[28,]<-c(PfuseLG.int)            #Probability of being large fusion intercept given that you survived and fused by site
params[29,]<-rep(PfuseLG.slope)        #probability of being a large fusion slope given that you survived and fused same across sites
params[30,]<-rep(clonesize.int)        #clone size intercept, same across sites
params[31,]<-rep(clonesize.slope)      #clone size slope, same across sites
params[32,]<-rep(clonesize.sd2)        #clone size sd, same across sites
params[33,]<-rep(clonesize.varFunc)    #clone size variance increases with size by site
params[34,]<-rep(clonenumlo.int)     #clonenum intercept lo number of fission events 
params[35,]<-rep(clonenumlo.slope)   #clonenum slope lo number of fission events
params[36,]<-rep(clonenumhi.int)     #clonenum intercept hi number of fission events
params[37,]<-rep(clonenumhi.slope)   #clonenum slope hi number of fission events
params[38,]<-rep(Philo.int)
params[39,]<-rep(Philo.slope)
params[40,]<-c(recruit.size.mean)      #mean size of recruits, same across sites
params[41,]<-rep(recruit.size.sd)      #sd size of recruits, same across sites
params[42,]<-c(r)
rownames(params)<-c("surv.int","surv.slope","growthngs.int","growthngs.slope","growthngs.sd2","growthngs.varFunc","growthes.int","growthes.slope","growthes.sd","growtheg.int","growtheg.slope","growtheg.sd","fiss.int","fiss.slope","fiss.sd","fuse.int","fuse.slope","fuse.sd2","fuse.varFunc","Pes.int","Pes.slope","Peg.int","peg.slope","Pfiss.int","Pfiss.slope","Pfuse.int","Pfuse.slope","PfuseLG.int","PfuseLG.slope","clonesize.int","clonesize.slope","clonesize.sd2","clonesize.varFunc","clonenumlo.int","clonenumlo.slope","clonenumhi.int","clonenumhi.slope","Philo.int","Philo.slope","recruit.size.mean","recruit.size.sd","r")
...
gngs.yx=function(xp,x,params) {
  sigmaxp2<-params[5,site]*exp(2*params[6,site]*x)
  sigmaxp<-sqrt(sigmaxp2)
  g<-dnorm(xp,params[3,site]+params[4,site]*x,sigmaxp)
}
ges.yx=function(xp,x,params) {
  g<-dnorm(xp,params[7,site]+params[8,site]*x,params[9,site])
}
geg.yx=function(xp,x,params) {
  g<-dnorm(xp,params[10,site]+params[11,site]*x,params[12,site])
}
ig.yx=function(xp,x,params) {
  g<-dnorm(xp,params[13,site]+params[14,site]*x,params[15,site])
}
ug.yx=function(xp,x,params) {
  sigmaxp2<-params[18,site]*exp(2*params[19,site]*x)
  sigmaxp<-sqrt(sigmaxp2)
  g<-dnorm(xp,params[15,site]+params[17,site]*x,sigmaxp)
}

#probability of extreme growth and shrinkage
p.es.x=function(x,params) {
  u=exp(params[20,site]+params[21,site]*x)
  return(u/(1+u))
}
p.eg.x=function(x,params) {
...
clo.yx=function(xp,x,params) {
  sigmaxp2<-params[32,site]*exp(2*params[33,site]*x)
  sigmaxp<-sqrt(sigmaxp2)
  nfiss<-exp(params[34,site]+params[35,site]*x)
  ...
  p.fiss.x(x,params)*p.hilo.x(x,params)*nfiss*dnorm(xp,params[30,site]+params[31,site]*x,sigmaxp)
}
chi.yx=function(xp,x,params) {
  sigmaxp2<-params[32,site]*exp(2*params[33,site]*x)
  sigmaxp<-sqrt(sigmaxp2)
  nfiss<-exp(params[36,site]+params[37,site]*x)
  ...
  p.fiss.x(x,params)*(1-p.hilo.x(x,params))*nfiss*dnorm(xp,params[30,site]+params[31,site]*x,sigmaxp)
}
...
f.yx=function(xp,x,params){
  params[42,site]*x*dnorm(xp,params[40,site],params[41,site])
}
...
Kernel<-function(y,n,params){
  ...
  ES=Gr*p.es.x(y,params=params)
  EG=Gr*p.eg.x(y,params=params)
  NGS=Gr*(1-p.es.x(y,params=params)-p.eg.x(y,params=params))
  GNGS=h*outer(y,y,gngs.yx,params=params)
  GES=h*outer(y,y,ges.yx,params=params)
  GEG=h*outer(y,y,geg.yx,params=params)
  GI=h*outer(y,y,ig.yx,params=params)
  GU=h*outer(y,y,ug.yx,params=params)
  
  
  Pngs=GNGS
  for (i in 1:n) Pngs[,i]=GNGS[,i]*S[i]*(NGS[i])
  Pes=GES
  for (i in 1:n) Pngs[,i]=GES[,i]*S[i]*(ES[i])
  Peg=GEG
  for (i in 1:n) Peg[,i]=GEG[,i]*S[i]*(EG[i])
  Pi=GI
