...
growth.reg<-gnls(sizeNext~a+b*size+c*exp(-d*size),data=growthonly,params=list(a+b~utrans,c+d~1),start=list(rep(1,22)),weights=varExp(form=~size))
summary(growth.reg)
growth.int<-c(coefficients(growth.reg)[1],coefficients(growth.reg)[1]+coefficients(growth.reg)[2:10])
growth.slope<-c(coefficients(growth.reg)[11],coefficients(growth.reg)[11]+coefficients(growth.reg)[12:20])
growth.e1<-coefficients(growth.reg)[21]
